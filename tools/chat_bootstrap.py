...
from knowledge_hub.services import build_chat_bootstrap_pack, render_chat_bootstrap_text


def main() -> int:
    parser = argparse.ArgumentParser(description="Export the shortest chat bootstrap brief for a project.")
    ...
    if args.format == "json":
        print(json.dumps(chat_bootstrap_pack, ensure_ascii=False, indent=2))
    else:
        print(render_chat_bootstrap_text(chat_bootstrap_pack))
    return 0
